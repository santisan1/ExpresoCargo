...
function openAlertsFor(alerts, packageId) {
  return alerts.filter((a) => a.packageId === packageId && a.status !== 'resolved')
}

function operatorRank(pkg, alerts) {
  if (pkg.urgency === 'urgente' && pkg.currentStatus !== 'delivered') return 1
  if (pkg.currentStatus === 'created') return 2
  if (pkg.currentStatus === 'received') return 3
  if (pkg.currentStatus === 'incident' || openAlertsFor(alerts, pkg.id).length > 0) return 4
  if (pkg.currentStatus === 'classified') return 5
  if (pkg.currentStatus === 'delivered') return 99
  return 10
}
...
export default function Packages() {
  ...
  const filtered = useMemo(() => {
    const base = packages.filter((p) => (!filters.text || `${p.guideNumber} ${p.barcodeValue} ${getDestination(p)}`.toLowerCase().includes(filters.text.toLowerCase())) && (!filters.status || p.currentStatus === filters.status) && (!filters.urgency || p.urgency === filters.urgency) && (!filters.zone || p.assignedZoneId === filters.zone))
    if (profile?.role !== 'operario') return base
    return [...base].sort((a, b) => operatorRank(a, alerts) - operatorRank(b, alerts) || String(a.guideNumber || '').localeCompare(String(b.guideNumber || ''), 'es'))
  }, [packages, filters, profile?.role, alerts])
  function exportCsv() {
    const today = new Date().toISOString().slice(0, 10)
    downloadCsv(`packages-erp-${today}.csv`, [['guideNumber','destinationCity','assignedZoneCode','urgency','currentStatus','lastScanAt','hasIncident','openAlerts'], ...filtered.map((p) => [p.guideNumber, getDestination(p), p.assignedZoneCode || p.assignedZoneId, p.urgency, p.currentStatus, formatDate(p.lastScanAt), Boolean(p.hasIncident), openAlertsFor(alerts, p.id).length])])
  }
  const dispatched = packages.filter((p) => p.currentStatus === 'dispatched')
  return <div className="page-stack"><div className="page-title"><div><h2>Listado de paquetes</h2><p>{profile?.role === 'operario' ? 'Cola operativa: paquetes pendientes de recepción, clasificación o revisión.' : 'Filtros operativos y exportaciones simuladas WMS/TMS/ERP.'}</p></div><div className="actions">{can(profile, 'packages.create', ['supervisor']) && <Link to="/packages/new" className="btn btn-primary">Nuevo paquete</Link>}<button className="btn" onClick={exportCsv}>Exportar CSV para ERP</button><button className="btn" onClick={() => exportOperationalExcel({ packages: filtered, alerts, zones, movements })}>Exportar Excel operativo</button><button className="btn" onClick={() => setRouteOpen(true)}>Generar hoja de ruta TMS</button></div></div>
