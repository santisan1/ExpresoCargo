...
import { downloadCsv, getDestination, getProductType, getWeight } from '../utils/format'

export default function Packages() {
  const { packages, alerts, zones } = useData(); const navigate = useNavigate()
  ...
  function exportCsv() { downloadCsv('packages-erp.csv', [['Guia','Destino','Estado','Urgencia','Zona','Peso'], ...filtered.map((p) => [p.guideNumber, getDestination(p), p.currentStatus, p.urgency, p.assignedZoneCode || p.assignedZoneId, getWeight(p)])]) }
  const dispatched = packages.filter((p) => p.currentStatus === 'dispatched')
  return <div className="page-stack"><div className="page-title"><div><h2>Listado de paquetes</h2><p>Filtros operativos y exportaciones simuladas WMS/TMS/ERP.</p></div><div className="actions"><Link to="/packages/new" className="btn btn-primary">Nuevo paquete</Link><button className="btn" onClick={exportCsv}>Exportar CSV para ERP</button><button className="btn" onClick={() => setRouteOpen(true)}>Generar hoja de ruta TMS</button></div></div>
