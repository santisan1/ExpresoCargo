...
import { can, hoursSince } from '../utils/format'
...
export default function Dashboard() {
  ...
  const urgent = packages.filter((p) => p.urgency === 'urgente')
  const wrongZone = packages.filter((p) => p.assignedZoneId === 'zona-incidencias' || p.assignedZoneCode === 'INC')
  const kpis = [
    ...
    ['Alertas abiertas', openAlerts.length, 'requieren acción', '⚠', 'red'], ['Urgentes', urgent.length, 'cross-docking', '⚡', 'orange'],
    ['Demorados', delayed.length, 'fuera de SLA', '⏳', 'red'], ['Zona incidencia', wrongZone.length, 'mal clasificados/revisar', 'INC', 'gray'],
  ]
  ...
  return <div className="page-stack"><div className="page-title"><div><h2>Dashboard de supervisión</h2><p>KPIs operativos en tiempo real con listeners de Firestore.</p></div>{can(profile, 'alerts.recalculate', ['supervisor']) && <button className="btn btn-primary" onClick={() => recalculateAlerts(packages, profile)}>Recalcular alertas</button>}</div>
