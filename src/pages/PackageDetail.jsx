...
export default function PackageDetail({ id }) {
  ...
  const downloadLabel = () => generatePackageLabelPdf(pkg, zones).catch((error) => console.error(error.message || error))
  return <div className="page-stack"><div className="page-title"><div><h2>Detalle de trazabilidad</h2><p>Guía {pkg.guideNumber} · QR/barra {pkg.barcodeValue || pkg.qrPayload}</p></div><div className="actions no-print"><button className="btn btn-primary" onClick={downloadLabel}>Descargar etiqueta PDF</button><StatusBadge status={pkg.currentStatus} /></div></div>
    <section className="grid two"><article className="card"><h3>Datos principales</h3><div className="detail-grid"><span>Estado actual</span><strong><StatusBadge status={pkg.currentStatus} /></strong><span>Destinatario</span><strong>{getRecipientName(pkg)}</strong><span>Origen</span><strong>{pkg.originCity || '—'}</strong><span>Destino</span><strong>{getDestination(pkg)}</strong><span>Zona esperada</span><strong>{getZoneLabel(pkg.expectedZoneId || pkg.assignedZoneId, zones)}</strong><span>Zona asignada</span><strong>{getZoneLabel(pkg.assignedZoneId || pkg.assignedZoneCode, zones)} · {pkg.assignedZoneCode || '—'}</strong><span>Ubicación actual</span><strong>{pkg.currentLocation || '—'}</strong><span>Último escaneo</span><strong>{pkg.lastScanAt ? formatDate(pkg.lastScanAt) : 'Sin escaneo'}</strong><span>Peso / Volumen / Bultos</span><strong>{getWeight(pkg)} kg · {getVolume(pkg)} m³ · {getBundles(pkg)}</strong><span>Producto</span><strong>{getProductType(pkg)}</strong><span>Urgencia</span><strong>{pkg.urgency || 'normal'}</strong><span>Etapa incidencia</span><strong>{pkg.incidentCheckpoint || pkg.incidentStage || '—'}</strong></div></article><article className="card"><h3>Acciones operativas</h3><OperationalActions pkg={pkg} /></article></section>
